...
class InputBindings:
    ...
    gamepad_buttons: dict[str, int] = field(
        default_factory=lambda: {
            "jump": 0,
            "attack1": 1,
            "attack2": 2,
    ...
    gamepad_axes: dict[str, int] = field(
        default_factory=lambda: {
            "move_x": 0,
            "dash": 2,
        }
    ...
    keyboard_combos: dict[str, list[int]] = field(
        default_factory=lambda: {
            "special_attack": [pygame.K_g, pygame.K_h],
        }
    )

    gamepad_combos: dict[str, list[int]] = field(
        default_factory=lambda: {
