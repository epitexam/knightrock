...
class Stage:
    ...
    * **The window is re-centred after every rebuild.** Changing resolution or
    ...
    def _create(
        ...
        self._recenter(surface.get_size(), resolved)
        return surface
    ...
    @staticmethod
    def _recenter(size: tuple[int, int], desktop: tuple[int, int]) -> None:
        if desktop[0] <= 0 or desktop[1] <= 0:
            return
        pygame.display.set_window_position(detection.centered_on_primary(size, desktop))
