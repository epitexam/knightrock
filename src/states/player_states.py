...
class PlayerBaseState(State):
    ...
    def ground_return(self) -> str:
        if self.entity.on_surface["floor"]:
            return "run" if (self.entity.left_held or self.entity.right_held) else "idle"
        return "fall"
...
class PlayerRunState(PlayerBaseState):
    def update(self, delta_time: float) -> Optional[str]:
        ...
        if not self.entity.on_surface["floor"]:
            return "fall"
        if not (self.entity.left_held or self.entity.right_held) and abs(self.entity.velocity.x) < 0.1:
            return "idle"
        return None
...
class PlayerBlockState(PlayerBaseState):
    ...
    def update(self, delta_time: float) -> Optional[str]:
        ...
        self.entity.block_stamina -= delta_time if self.entity.on_surface["floor"] else delta_time * 2.0
...
class PlayerHurtState(PlayerBaseState):
    def update(self, delta_time: float) -> Optional[str]:
        control = self.entity.floor_control if self.entity.on_surface["floor"] else self.entity.air_control
        self.entity.velocity.x = pygame.math.lerp(self.entity.velocity.x, 0.0, min(1.0, control * delta_time))
    ...
    def exit(self, next_state: Optional[str] = None) -> None:
        if hasattr(self.entity, "_original_hitbox_width"):
            ...
            if current != self.entity._original_hitbox_width:
                self.entity.hitbox.x -= (self.entity._original_hitbox_width - current) / 2
